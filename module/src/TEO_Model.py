...
def buildmodel(sets_df, df, defaults_df, mcs_df, n):
    ...
    FUEL = createTuple(sets_df["FUEL"], "FUEL")
    EMISSION = createTuple(sets_df["EMISSION"], "EMISSION")
    MODE_OF_OPERATION = createTuple(sets_df["MODE_OF_OPERATION"], "MODE_OF_OPERATION")
    REGION = createTuple(sets_df["REGION"], "REGION")
    REGION2 = createTuple(sets_df["REGION2"], "REGION2")
    SEASON = createTuple(sets_df["SEASON"], "SEASON")
    DAYTYPE = createTuple(sets_df["DAYTYPE"], "DAYTYPE")
    DAILYTIMEBRACKET = createTuple(sets_df["DAILYTIMEBRACKET"], "DAILYTIMEBRACKET")
    FLEXIBLEDEMANDTYPE = createTuple(sets_df["FLEXIBLEDEMANDTYPE"], "FLEXIBLEDEMANDTYPE")
    STORAGE = createTuple(sets_df["STORAGE"], "STORAGE")

    logging.info(
        f"\t{dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\t" f"Sets are created."
    )
    ...
    while i <= n:
        ...
        variables = {
           ...
           'DiscountedSalvageValueByTechnology': {'sets': [REGION, TECHNOLOGY], 'lb': 0, 'ub': None, 'cat': 'Continuous', 'indices': ['r', 't']},
           'OperatingCost': {'sets': [REGION, TECHNOLOGY, YEAR], 'lb': 0, 'ub': None, 'cat': 'Continuous', 'indices': ['r', 't', 'y']},
           'VariableOMCost': {'sets': [REGION, MODE_OF_OPERATION, TECHNOLOGY, YEAR], 'lb': 0, 'ub': None, 'cat': 'Continuous', 'indices': ['r', 'm', 't', 'y']},
           'DiscountedOperatingCost': {'sets': [REGION, TECHNOLOGY, YEAR], 'lb': 0, 'ub': None, 'cat': 'Continuous', 'indices': ['r', 't', 'y']},
           'DiscountedOperatingCostByTechnology': {'sets': [REGION, TECHNOLOGY], 'lb': 0, 'ub': None, 'cat': 'Continuous', 'indices': ['r', 't']},
           ...
           'DiscountedAnnualFixedOperatingCost': {'sets': [REGION, TECHNOLOGY, YEAR], 'lb': 0, 'ub': None, 'cat': 'Continuous', 'indices': ['r', 't', 'y']}, 
           'TotalDiscountedFixedOperatingCost': {'sets': [REGION, TECHNOLOGY], 'lb': 0, 'ub': None, 'cat': 'Continuous', 'indices': ['r', 't']}, 
        ...
        DiscountedAnnualFixedOperatingCost = createVariable('DiscountedAnnualFixedOperatingCost', variables)
        TotalDiscountedFixedOperatingCost = createVariable('TotalDiscountedFixedOperatingCost', variables)
        ...
        VariableOMCost = createVariable('VariableOMCost', variables)
        # ====  Reserve Margin  ====
        ...
        for rflmty in REGION_FUEL_TIMESLICE_MODE_OF_OPERATION_TECHNOLOGY_YEAR:
            ...
            if InputActivityRatio.get(ci([*rflmty[0:2], *rflmty[3:6]]), dflt.get('InputActivityRatio')) != 0: 
                model += RateOfUseByTechnologyByMode.get(ci(rflmty)) == RateOfActivity.get(ci([rflmty[0], *rflmty[2:6]])) * ( 1 / InputActivityRatio.get(ci([*rflmty[0:2], *rflmty[3:6]]), dflt.get('InputActivityRatio'))), ""
    ...
    #         model += RateOfStorageDischarge.get(ci(rldlhlssy)) <= StorageMaxDischargeRate.get(ci(rldlhlssy[4:6]), dflt.get('StorageMaxDischargeRate')), ""
        ...
        for rsy in REGION_STORAGE_YEAR:
            if(StorageL2D.get(ci(rsy[0:2]), dflt.get('StorageL2D')) == 0):
                 model += StorageSurfaceArea.get(ci(rsy)) == 0.0744 * AccumulatedNewStorageCapacity.get(ci(rsy)) * Storagetagheating.get(ci(rsy[0:2]), dflt.get('Storagetagheating')) + 0.0361 * AccumulatedNewStorageCapacity.get(ci(rsy)) * Storagetagcooling.get(ci(rsy[0:2]), dflt.get('Storagetagcooling')), ""  
            elif(StorageL2D.get(ci(rsy[0:2]), dflt.get('StorageL2D')) == 1):
                 model += StorageSurfaceArea.get(ci(rsy)) == 0.1339 * AccumulatedNewStorageCapacity.get(ci(rsy)) * Storagetagheating.get(ci(rsy[0:2]), dflt.get('Storagetagheating')) + 0.065 * AccumulatedNewStorageCapacity.get(ci(rsy)) * Storagetagcooling.get(ci(rsy[0:2]), dflt.get('Storagetagcooling')), ""
...
#             model += StorageMaxDischargeRate.get(ci([*rsly[0:2], rsly[3]]), dflt.get('StorageMaxDischargeRate')) >= StorageLevelTimesliceStart.get(ci([*rsly[0:2], str(int(rsly[2])-1), rsly[3]])) - StorageLevelTimesliceStart.get(ci(rsly)), ""
    ...
    #         # SI5.1_DiscountingCapitalInvestmentbyStorageBusinessModule
        ...
        # ====  Salvage Value  ====
            ...
            model += DiscountedAnnualFixedOperatingCost.get(ci(rty)) == AnnualFixedOperatingCost.get(ci(rty)) * (1 / ((1 +  DiscountRateTech.get(ci(rty[0:2]), dflt.get('DiscountRateTech'))) ** (int(rty[2]) - int(min(YEAR)) + 0.5))), ""
        ...
        for rmty in REGION_MODE_OF_OPERATION_TECHNOLOGY_YEAR:
            model += VariableOMCost.get(ci(rmty)) == 1 * VariableCost.get(ci(rmty), dflt.get('VariableCost')), ""

        # ====  Business module Discounted Costs for storage and Technology  ==== 
        # ====  Technology  ====   
        for rt in REGION_TECHNOLOGY:
    #         # CC2.1_DiscountingCapitalInvestmentbytechnology
             model += DiscountedCapitalInvestmentByTechnology.get(ci(rt)) ==  pulp.lpSum([DiscountedCapitalInvestment.get(ci([*rt, y])) for y in YEAR]), ""
    #         # OC4.1_DiscountedOperatingCostsbytechnologyBusinessModule
             model += DiscountedOperatingCostByTechnology.get(ci(rt)) ==  pulp.lpSum([DiscountedOperatingCost.get(ci([*rt, y])) for y in YEAR]), ""   
